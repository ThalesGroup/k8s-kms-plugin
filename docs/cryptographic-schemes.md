...
sequenceDiagram
    ...
    participant HSM as HSM / TPM (KEK)
...
```json
{
  "ciphertext": "ZXlKaGJHY2lPaUprYVhJaUxDSnJhV1Fp…<216 chars elided>… N2tuTjJpaDFfbkhB",
  "keyId": "01",
  "annotations": {
...
```json
{
  "ciphertext": "ZXlKaGJHY2lPaUpCTWpVMlEwSkRJaXdp…<348 chars elided>… R01PTlpNLTRiWlJn",
  "keyId": "02",
  "annotations": {
...
| RSA key size | `EncryptResponse.ciphertext` | % of the 1 kB limit |
|---|---|---|
| RSA-2048 | 602 B | 59% |
| RSA-3072 | 772 B | 75% |
| RSA-4096 | 943 B | 92% |

> ⚠️ **RSA-4096 is close to the ceiling.** At 92% there is ~80 B of headroom, and the figure is
...
> 📐 These figures were captured before the gose v1.0.0-rc2 upgrade, when the header read
> `RSA-OAEP`. `RSA-OAEP-256` is 4 bytes longer in JSON, so each row grows by ~6 B once the
> protected header is base64url-encoded — RSA-4096 lands near 949 B (93%). Re-run
> [`collect-jwe-samples.sh`](../scripts/grpcurl/) to refresh the table with measured values.
...
```json
{
  "ciphertext": "ZXlKaGJHY2lPaUpTVTBFdFQwRkZVQ0lz…<804 chars elided>… Y080S29ET1hiLXc=",
  "keyId": "04",
  "annotations": {
...
(`e082b848…d2e0a10d`), not the short `CKA_ID` used as `key_id` — the two are independent.

⚠️ This capture predates the gose v1.0.0-rc2 upgrade: `ZXlKaGJHY2lPaUpTVTBFdFQwRkZVQ0lz` decodes to
`eyJhbGciOiJSU0EtT0FFUCIs`, i.e. `{"alg":"RSA-OAEP",`. A fresh capture will show
`{"alg":"RSA-OAEP-256",`.
...
5. AES-GCM-seal the DEK seed under the derived key.
6. Emit the envelope.
...
same AES key, and open the AEAD. Because the key pair is resolved from `key_id`, the rotation server
can decrypt objects written under the previous KEK.
...
Total annotation footprint is ≈ 1.7 kB against the 32 kB budget.
...
```json
{
  "ciphertext": "Sp4r1hO1I7nbXtRADhGTcgUjjORvZjppL/OKSThnF3JBf8pQ4NUPpkBnJ4BQJmTiLWVlPXaJtnpsntkt",
  "keyId": "09",
  "annotations": {
    "algorithm-family.k8s-kms-plugin.keysealer.eclipse.org": "bWwta2Vt",
    "kem-ciphertext.k8s-kms-plugin.keysealer.eclipse.org": "DhBvO2PK66+Etco8E8LAKZNQfd4Hix8v…<2092 chars elided>… WWd9sAL/yu9HMlo="
  }
}
```
...
Tampering is also handled, if implicitly: substituting a different `c` makes `ML-KEM.Decaps` return a
*different* shared secret (FIPS 203 uses implicit rejection rather than returning an error), which
derives a different AES key, which fails the AES-GCM tag check. The decrypt fails cleanly.
