...
Using both environment variables and configuration file and serving on unix socket and serving on unix socket:
...
```
  ...
  -p, --native-path string       Path to key store for native provider(Files only). Env var: K8S_KMS_PLUGIN_SERVE_NATIVE_PATH. (default ".keys")
      ...
      --require-client-cert      Require and verify client certificate for mTLS. Env var: K8S_KMS_PLUGIN_SERVE_REQUIRE_CLIENT_CERT
      ...
      --tls-client-ca string     TLS CA cert. Env var: K8S_KMS_PLUGIN_SERVE_TLS_CLIENT_CA (default "certs/ca.crt")
