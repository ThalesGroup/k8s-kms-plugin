...
function init(){
    ...
    if [ "${dev_mode}" = true ]; then
      ...
      eval GOPROXY=direct go get -u "github.com/eclipse-keypont/crypto11/v2@${branch_name}"
      eval GOPROXY=direct go get -u "github.com/eclipse-keypont/gose@${branch_name}"
      go mod tidy
    fi
